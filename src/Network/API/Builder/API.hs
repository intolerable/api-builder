...
type API s e a = APIT s e a IO a
...
instance MonadTrans (APIT s e r) where
  lift = APIT . lift . lift . lift . lift . lift
...
liftExcept :: Monad m => ExceptT (APIError e, APIT s e r m r) (ReaderT Manager (StateT Builder (StateT s m))) a -> APIT s e r m a
liftExcept = APIT . lift

{-# DEPRECATED liftEither "Use liftExcept" #-}
-- | Identical to 'liftExcept', provided for (almost) compatibility.
liftEither :: Monad m => ExceptT (APIError e, APIT s e r m r) (ReaderT Manager (StateT Builder (StateT s m))) a -> APIT s e r m a
liftEither = APIT . lift

-- | Lifts an action that works on a @Manager@ to one that works on an @API@.
liftManager :: Monad m => ReaderT Manager (StateT Builder (StateT s m)) a -> APIT s e r m a
liftManager = APIT . lift . lift
...
liftBuilder :: Monad m => StateT Builder (StateT s m) a -> APIT s e r m a
liftBuilder = APIT . lift . lift . lift

-- | Lifts an action on an @API@'s state type @s@ to one that works on the @API@. Good
--   for messing with the state from inside the @API@.
liftState :: Monad m => StateT s m a -> APIT s e r m a
liftState = APIT . lift . lift . lift . lift
...
execAPI :: MonadIO m
       ...
       -> APIT s e a m a -- ^ the actual @API@ to run
...
runResumeAPI :: MonadIO m
             ...
             -> s
             -> APIT s e a m a
             -> m (Either (APIError e, APIT s e a m a) a, Builder, s)
runResumeAPI b m s (APIT api) = do
  ((res, b'), s') <- runStateT (runStateT (runReaderT (runExceptT (runContT api return)) m) b) s
  return (res, b', s')

-- | Runs a @Route@. Infers the type to convert to from the JSON with the @a@ in @API@,
--   and infers the error type from @e@.
runRoute :: (Receivable a, ErrorReceivable e, MonadIO m) => Route -> APIT s e r m a
runRoute = sendRoute ()

-- | Runs a @Route@, but only returns the response and does nothing towards
--   decoding the response.
routeResponse :: (MonadIO m, ErrorReceivable e) => Route -> APIT s e r m (Response ByteString)
...
sendRoute :: (MonadIO m, Sendable t, ErrorReceivable e, Receivable a) => t -> Route -> APIT s e r m a
sendRoute s r = do
  builder <- liftBuilder get
  manager <- liftManager ask
  APIT $ do
    label <- fmap APIT $ mkLabel
...
name :: Monad m => Text -> APIT s e r m ()
...
baseURL :: Monad m => Text -> APIT s e r m ()
...
--   for example.
customizeRoute :: Monad m => (Route -> Route) -> APIT s e r m ()
...
--   for example.
customizeRequest :: Monad m => (Request -> Request) -> APIT s e r m ()
